...
**mic-памп и peer-сплиттер** тоже больше не блокируются на MainActor
(health-стампы — неблокирующий Task, коалесc ≤1/0.5с).
...
- [x] Per-frame MainActor hop в pump'ах оркестратора — закрыт полностью (2026-07-02): output-пампы (PR #15), mic-памп и peer-сплиттер (блокирующий `MainActor.run` на каждом кадре → неблокирующий Task, коалесc ≤1/0.5с)
