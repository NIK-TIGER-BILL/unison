...
|---|---|---|
| `targetBufferSec` | 0.30 | Setpoint ≈ наблюдаемому p90 меж-чанковому jitter; рейт мягко сливает к нему, latency не добавляет |
...
лишь чтобы слить буфер выше `targetBufferSec` (0.30 с, ≈ p90 jitter), мягко
(до 1.15×), а быстрый сглаживатель (τ≈0.6с) ловит суб-секундные бёрсты.
`arrivalRateEMA` **убран из формулы** (остался только для диагностического лога).
На том же реальном timeline: underrun ↓ до **0.8%** (1 неизбежное окно — пауза
сети 505 мс после замедления модели), пик latency ↓ **963→550 мс**, рейт в
**[1.00, 1.05]×** (без слышимого растяжения), mean latency **без изменений**
(~315 мс). **Не покрывает:** если модель отдаёт медленнее реалтайма
*продолжительно* — буфер опустеет и будет underrun; мы сознательно **не**
замедляемся ниже 1.0× (это вернуло бы «робота» ради худшего трейда). Реальный
arrival ≈ 1.0× с jitter, так что это редко. `pipelineFrameBuffer=50` +
...
- [ ] Остаточный underrun на крупной (≥505мс) паузе сети после продолжительного замедления модели — неустраним без роста baseline latency (юзер не хочет); проверять на реальных звонках по логам `[UNDERRUN speakers]`
