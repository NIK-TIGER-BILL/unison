...
|---|---|---|
| `targetBufferSec` | **1.0** (env `UNISON_BUFFER_MS`) | **Потолок** джиттер-буфера: контроллер играет 1.0× пока очередь ≤ него (ДЕРЖИТ подушку), сливает только ВЫШЕ (cap latency). Не «тонкий setpoint, к которому сливаем» — это и был баг. Тюнится вживую |
