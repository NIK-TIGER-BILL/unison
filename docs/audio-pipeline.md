...
> research-агента, не измерялся). Осталось не сделано: **gap concealment**
> (NetEq/WSOLA — в паузе тянуть питч-период + comfort noise вместо тишины;
> «правильное» лекарство остаточных микропауз, но большая правка) — и **локализация
> «внезапных деградаций»**: модель это или наш pipeline (capture реального
> model-output через pacing-eval + numeric-scan на glitch/dropout/clip).
