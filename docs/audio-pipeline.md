...
|---|---|---|
| `targetBufferSec` | **0.60** (env `UNISON_BUFFER_MS`) | Подушка джиттер-буфера: контроллер держит очередь на этой глубине (сливает только выше). Больше = меньше фризов, но столько же latency. Тюнится вживую |
| `correctionGain` | 0.4 | `rate = 1.0 + (depth_smooth − setpoint)·gain` |
| `maxRate` | 1.15 | **Мягкий** cap слива — выше TimePitch слышно растягивает («робот») |
...
**Джиттер-буфер (`targetBufferSec`, реальные логи 2026-06-30).** v5 убрал
осцилляцию, но **фризы остались и были заметны**. Реальный лог показал причину:
**модель сама подвисает на 700–970 мс несколько раз за звонок** (`[audio-rx]`
gaps p90≈0.33с, max≈0.83с; `[sched-stall]` 63 шт, max 970 мс), а тонкая подушка
0.30 с это не переживает → ~6.7% фризов в 2 кластерах (реплей реального timeline
из лога офлайн). Pre-roll **не помогает** (контроллер сливает его к target) —
единственный рычаг это **сама подушка** `targetBufferSec`. Frontier на реальной
сессии: 0.30→6.7%/370мс, 0.50→5.5%, 0.75→4.6%, 1.0→3.8%/600мс (насыщается —
кластеры требуют active rebuild). Фундаментальный треугольник на реальных данных:
**низкая latency ↔ нет фризов ↔ нет «робота» — выбери 2** (нельзя играть аудио,
которое ещё не пришло). Выбран **latency**: подушка поднята до **0.60 с** по
умолчанию, тюнится вживую через `UNISON_BUFFER_MS` (=900 для рваной сети, =400
ради latency). Большой ~0.9 с стол намеренно **не** прячем целиком (слишком
много latency) и **не** растягиваем замедлением (вернёт «робота») — редкий
большой стол остаётся коротким фризом. `pipelineFrameBuffer=50` +
`bufferingOldest` гарантируют что буфер не растёт > 5 с.
...
| `UNISON_BUFFER_MS=600` | Подушка джиттер-буфера (`PlaybackPacing.targetBufferSec`) в мс. Больше = меньше фризов на рваной сети, но +столько latency. Default 600 |
...
- [x] **Заметные фризы воспроизведения** — реальный лог: модель подвисает на 700–970 мс (`[sched-stall]` 63 шт), тонкая подушка 0.30с не переживает (~6.7% фризов). Поднята `targetBufferSec`→0.60 (env `UNISON_BUFFER_MS`), джиттер-буфер. Это latency↔smoothness дайл — калибруется вживую под сеть
- [ ] Кластеры фризов (несколько столов подряд за ~2с) насыщают fixed-подушку на ~3.8% — следующий шаг если мало: **adaptive jitter buffer** (растить подушку только при детекте турбулентности, latency низкая в спокойное время) ИЛИ gentle active rebuild (slow до 0.96×, риск «робота»). Только если live-лог покажет что 0.60с недостаточно
