...
### `PlaybackPacing` v4 — bidirectional regulator (Sources/UnisonAudio/PlaybackPacing.swift)
Регулятор адаптивного TimePitch.rate вокруг **тонкого** буфер-setpoint.

| Константа | Значение | Смысл |
|---|---|---|
| `targetBufferSec` | 0.15 | Setpoint буфера ≈ наблюдаемой глубине (0.08–0.15 с) → latency не растёт |
| `correctionGain` | 0.4 | Отклик регулятора вокруг setpoint |
| `maxRate` | 1.5 | Cap на ускорение (слив бёрста) |
| `minRate` | 0.85 | Floor: можно играть чуть медленнее реалтайма, чтобы матчить arrival |
| `maxRateStepPerTick` | 0.05 | Slew = 0.5/сек — плавно |

**Почему v4 (был v3 «только ускорение»).** v3 фиксировал `minRate=1.0` и целил
буфер в 1.0 с → на тонких реальных глубинах был no-op; а так как модель отдаёт
**0.92–0.99× wall-clock** (`arrival_rate_ema`), проигрывание жёстко на 1.0×
**опустошало буфер до тишины** — это симптом «звук резко пропадает и
появляется». v4 регулирует очередь вокруг **тонкого setpoint** (≈ текущая
latency) и **разрешает играть чуть медленнее** (до 0.85×), чтобы consumption
матчил arrival, а не обгонял его.

**Latency не растёт:** матчинг arrival держит буфер стабильным на setpoint;
к floor рейт уходит лишь транзиентно — добрать критически пустой буфер, — а
buffer-error + `maxRate` сливают любой бёрст. Замер на живом Gemini: rate
≈ 0.93 (mean, до 0.85), depth ≈ 0.156 с (= setpoint), underrun ≈ 3% (остаток на
крупных меж-бёрстовых паузах — цена нулевой добавочной latency). `pipelineFrameBuffer=50`
+ `bufferingOldest` по-прежнему гарантируют что буфер не растёт > 5 с.
...
- [x] **«Звук резко пропадает и появляется»** — был underrun из-за `minRate=1.0` floor против sub-realtime arrival; чинит `PlaybackPacing` v4 (bidirectional, матчит arrival без роста latency)
- [ ] Остаточный ~3% underrun на крупных меж-бёрстовых паузах при тонком буфере — сглаживать фейдами на границах (latency-neutral), если будет слышно на реальных звонках
